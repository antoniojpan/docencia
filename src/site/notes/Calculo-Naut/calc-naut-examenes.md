---
{"dg-publish":true,"permalink":"/calculo-naut/calc-naut-examenes/","updated":"2026-01-09T11:22:12.852+01:00"}
---
...
1. Límites:
    ...
    - $\lim_{n\rightarrow\infty}\sqrt{4n^{2}-3n}-2n$
        
    - $\lim_{x\rightarrow0}\left(\frac{2-2\cos(x)}{x^{2}}\right)^{\frac{8}{x^{2}}}$
        
2. Derivabilidad:
    
    Estudiar la derivabilidad de la función según los valores de $a$:
    
    $$f(x)=\begin{cases}\frac{4}{x+1} & x<a \\ \frac{11-2x-x^{2}}{4} & a\le x\end{cases}$$
...
2. Límites:
    ...
    - a) $\lim_{x\rightarrow+\infty}\left(\frac{2x^{2}+3}{3x}-\frac{4x+3}{2}\right)$
        
    - b) $\lim_{x\rightarrow+\infty}\left(\frac{\sqrt{x^{2}-2}-x}{2}\right)$
        
3. Continuidad:
    
    Estudiar la continuidad de:
    
    $$f(x)=\begin{cases}\frac{1}{x-1} & x<2 \\ 2x-3 & 2\le x\end{cases}$$
...
2. Límites:
    ...
    - $\lim_{x\rightarrow0} \frac{x\cosh(x)+b \sinh(x)}{x^3}$ sabiendo que es finito.
        
    - $\lim_{x\rightarrow+\infty} (x^2+2x-x)^x$ (Nota: posible errata en el original, comúnmente $\sqrt{x^2+2x}-x$).
        
3. Continuidad y Derivabilidad:
    
    Estudiar la continuidad y derivabilidad de:
    
    $$f(x)=\begin{cases}\frac{1}{x+4} & x<a \\ -x-6 & a\le x\end{cases}$$
...
1. Continuidad:
    ...
    $$f(x)=\begin{cases}2^{1/x} & \text{si } x<0 \\ \frac{x}{x^{2}-1} & \text{si } 0\le x<1 \\ 0 & \text{si } x>1\end{cases}$$
...
3. Límite:
    ...
    Calcula el siguiente límite: $\lim_{n\rightarrow\infty}\left(\frac{n^{2}+\sqrt[3]{n^{2}+n+1}}{n^{2}-5}\right)^{n}$
...
2. Integrales dobles:
    ...
    - a) $\int_{0}^{1}\int_{0}^{x}e^{-x^{2}}dy~dx$
        
    - b) $\int_{-1}^{1}\int_{0}^{\arctan(x)}\frac{x^{3}+1}{\cos^{2}(y)}dy~dx$
        
3. Derivadas parciales:
    
    Dada $f(x,y)=xy+xe^{y/x}$, calcula $\frac{\partial^{2}f}{\partial x^{2}}$.
...
2. Derivabilidad:
    ...
    Estudiar la derivabilidad de $f(x)=\begin{cases}4x+1 & x<a \\ \frac{11-2x-x^{2}}{4} & a\le x\end{cases}$ según valores de $a$.
...
4. Multiplicadores de Lagrange:
    ...
    Estudiar los máximos y mínimos relativos de la función $f(x,y)= 4x^2+9y^2-x^2y^2$ con la condición de que $x^2+y^2=4$.
